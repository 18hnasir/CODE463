...
import Homework8(SnocList(..),snocLast,snocProduct,snocMax,longestSnocSuffix,
                 ...
                 tribM,trib,parti,partitionM,partition,
                 simpleBalanced,balancedM,balanced,
...
test_Tree = TestList [
  -- Eq
  tc "Tree "  True  $ (==) (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L)))  (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L))) ,
  -- one buried value is different.
  tc "Tree "  False $ (==) (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L)))  (Br (V 100) 2 (Br (V 3) 4 (Br L 5 L))) ,
  -- different trees, even though the same inorder walk exists.
  tc "Tree "  False $ (==) (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L)))  (Br (V 1) 2 (Br (V 3) 4 (V 5))),
  tc "Tree "  True $ and [L==(L::Tree Int), (V 3)==(V 3), (Br L 7 L)==(Br L 7 L)],
  
  --Ord
  tc "Tree "  LT $ compare (Br (V 1) 10 (V 100)) (Br (V 5) 10 (V 15)),
  tc "TRee "  EQ $ compare (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L))) (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L))),
  tc "Tree "  GT $ compare (Br (V 5) 10 (V 15))  (Br (V 1) 10 (V 100)) ,
  -- read our spec carefully for the next two cases' reasoning.
  tc "Tree "  LT $ compare (V 5) (Br L 5 L),
  tc "Tree "  GT $ compare (Br L 100 L) (V 9001)
  ]

test_insertTree = TestList [
  tc "insertTree " (Br (Br L 1 (V 2)) 3 (Br (V 4) 5 L))  $ insertTree 4 (Br (Br L 1 (V 2)) 3 (V 5)),
  tc "insertTree " (Br (Br L 1 (V 2)) 3 (Br L 5 (V 6)))  $ insertTree 6 (Br (Br L 1 (V 2)) 3 (V 5)),
  tc "insertTree " (Br (V 3) 4 L)  $ insertTree 3 (V 4),
  tc "insertTree " (V 5) $ insertTree 5 L
  ]

test_inOrder = TestList [
  tc "inOrder " [1,2,3,4,5] $ inOrder  (Br (V 1) 2 (Br (V 3) 4 (Br L 5 L))),
  tc "inOrder " [1,2,3]     $ inOrder  (Br (Br L 1 L) 2 (Br L 3 L)),
  tc "inOrder " [10] $ inOrder  (V 10)
  
  ]

test_treeSort = TestList [
  tc "treeSort " [1,2,3,4,5] $ treeSort  [1,5,3,2,4],
  tc "treeSort " [2,4,4,6,6] $ treeSort  [2,6,4,4,6],
  tc "treeSort " [] $ ([] :: [Int])
  ]

test_treeMin = TestList [
  tc "treeMin " (Just 2) $ treeMin (Br (V 10) 2 (Br (V 3) 4 (V 5))),
  tc "treeMin " (Just (-5)) $ treeMin (Br (V 1) 2 (Br (V 3) 4 (Br (V (-5)) 6 (V 7))))
  ]
...
test_parti = TestList [
  tc "partition 0" ([2,4,6],[1,3,5]) $ parti even [1,2,3,4,5,6],
  tc "partition 1" ([4,3,2],[8,7,6,5]) $ parti (<5) [8,7,6,5,4,3,2],
  tc "partition 2" ([],[1,3,5]) $ parti even [1,3,5],
  tc "partition 3" ([1,3,5],[]) $ parti odd [1,3,5]
  ]

swap (a,b) = (b,a)
test_partition = TestList [
  tc "partitionM 0" ([2,4,6],[1,3,5]) $ swap $ runState (partitionM even [1,2,3,4,5,6]) [],
  tc "partitionM 1" ([1000,2,4,6],[1,3,5]) $ swap $ runState (partitionM even [1,2,3,4,5,6]) [1000],
  tc "partitionM 2" ([4,3],[7,6,5]) $ swap $ runState (partitionM (<5) [7,6,5,4,3]) [],
  tc "partition 3" ([],[1,3,5]) $ partition even [1,3,5] ,
  tc "partition 4" ([(5,3),(10,1)],[(6,7),(2,2)]) $ partition (\(a,b)->a>b) [(5,3), (6,7),(10,1),(2,2)]
  ]
...
test_unitTriangles = TestList [
  tc "unitTriangles " [(1,1,1),(1,2,2),(1,3,3),(1,4,4),(2,2,2),(2,2,3),(2,3,3),(2,3,4),(2,4,4),(3,3,3),(3,3,4),(3,4,4),(4,4,4)]
